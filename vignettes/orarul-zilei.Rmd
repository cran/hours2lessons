---
title: "Orarul zilei într-o școală"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Orarul zilei într-o școală}
...
Admitem și existența unor *tuplaje*, într-un set suplimentar de date: profesorii din acest set au de intrat în câte o aceeași oră a zilei, la câte o grupă constituită din elevi de la două, trei sau chiar patru clase (sau eventual, la câte una dintre clasele respective); formal, un tuplaj este reprezentat inițial înscriind în `prof` vectorul profesorilor și în `cls` vectorul claselor "tuplate" acestora. Ca *exemplu* este furnizat setul de lecții tuplate **dayTuples**.

```{r}
dayTuples
```

De observat că dacă numărul de profesori este mai mare ca al claselor (în linia 3 ar fi 4 profesori, pe 3 clase), atunci... avem de înființat *noi* cuplaje (de exemplu, `Fr1Fr3` pe clasa `10B`), încât profesorii și clasele din tuplaj să fie în relație "one-to-one"; dar utilizatorul este cel care știe care profesori ar trebui cuplați, încât lui îi revine sarcina înființării de noi cuplaje, dacă este necesar, pentru a introduce tuplajele corecte.
...
Mai întâi, se verifică dacă profesorii și clasele au nu mai mult de 7 ore și dacă tuplajele (când există) sunt definite corect.
...
De *exemplu*, pentru seturile **`dayLessons`** și **`dayTuples`** găsim orarul profesorilor angajați în cuplaje sau în tuplaje (cu sublinierea că la o nouă execuție, vom obține un *alt* orar, poate în mai puține încercări, poate mai bun sau poate, mai rău):
...
Profesorii `Fr1`, `Fr2` și `Gr1` formau un tuplaj pe clasele `11C` și `11E` și observăm că s-a înființat profesorul fictiv `Fr1Fr2` care intră la `11C` într-o aceeași oră în care `Gr1` intră la `11E` (oră în care `Fr1` și `Fr2` nu au de făcut alte lecții).\
`Fr1` și `Fr2` intră încă într-un tuplaj, cu `Fr3` și `Gr2`, pe clasele `10B`, `10C` și `10E`; utilizatorul care va fi definit tuplajele corecte, a înființat cuplajul `Fr1Fr3` pe clasa `10B`; se vede pe coloanele orare de mai sus că lecțiile acestui tuplaj cad într-o aceeași oră și nu există suprapuneri de lecții ale membrilor tuplajului.\
